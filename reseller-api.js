// ══════════════════════════════════════════════════════════════════
// reseller-api.js — Integrasi dengan Reseller API V2 VIP Best Mods
// (vipbestmods.com -- sebelumnya dikenal sebagai vipibmstore.com, domain
// lama tetap didukung lewat baseUrl kalau kredensial belum dipindah)
//
...
//
// PENTING -- dua istilah "stockMode"/"stock_mode" yang BEDA KONTEKS:
//   - product.stockMode (field kita sendiri): 'manual' | 'auto' -- nentuin
//     apakah key diambil dari pool kita, atau dari provider ini.
//   - item.stock_mode dari GET /v2/products (field provider): 'v2' | 'manual'
//     -- nentuin apakah item itu unlimited di sisi MEREKA, atau harus dicek
//     GET /v2/stock dulu sebelum order (lihat assertItemPurchasable).
//
...
//   canonical = METHOD + "\n" + path + "\n" + timestamp + "\n" + nonce + "\n" + SHA256(body)
//   secretHash = SHA256(apiSecret) [hex string]
...
// di-regenerate dari dashboard vipbestmods.com/reseller sebelum dipakai.
// ══════════════════════════════════════════════════════════════════
...
function getStock(settings, itemIds) {
  const ids = (Array.isArray(itemIds) ? itemIds : [itemIds]).map(String).filter(Boolean);
  if (ids.length === 0) {
    return Promise.resolve({ success: false, code: 'INVALID_ARGUMENT', message: 'itemIds tidak boleh kosong.' });
  }
  if (ids.length > 100) {
    return Promise.resolve({ success: false, code: 'INVALID_ARGUMENT', message: 'Maksimal 100 item_ids per panggilan GET /stock.' });
  }
  return doRequest(settings, { method: 'GET', relativePath: `/v2/stock?item_ids=${encodeURIComponent(ids.join(','))}` });
}

// Aturan stock provider (WAJIB dipatuhi sebelum orderKey()):
...
// productStockMode diambil dari katalog (getProductsCached()) -- kalau
// belum diketahui (undefined), default aman-nya anggap 'v2' (unlimited),
// SAMA seperti behavior sebelum endpoint /stock ada, supaya tidak
// tiba-tiba nge-block order yang sebelumnya jalan normal.
async function assertItemPurchasable(settings, providerItemId, productStockMode) {
  if (productStockMode !== 'manual') {
    return { purchasable: true };
  }
  const stockResult = await getStock(settings, [providerItemId]);
...
module.exports = { isConfigured, getConfig, getBalance, getProducts, getOrder, getStock, assertItemPurchasable, orderKey, debugBalance, resolveItemId, getProductsCached, getRealStockForProduct, setProductsCache, errorToCustomerMessage };
...
// SENGAJA tidak ada di tabel ini -- itu bug di sisi kita (canonical
// string, jam server, atau nonce kepakai ulang), BUKAN kesalahan customer.
// Kode tersebut selalu di-log dan dibalas pesan generik.
// ══════════════════════════════════════════════════════════════════
const SENSITIVE_PROVIDER_CODES = new Set(['INVALID_SIGNATURE', 'INVALID_TIMESTAMP', 'REPLAY_DETECTED']);

function errorToCustomerMessage(code, rawMessage) {
